...
VoidLotus_Dots_DIR="$HOME/Hyprland-Dots"

# exit if cannot find local version
if [ -z "$local_version" ]; then
  notify-send -i "$iDIR/error.png" 'ERROR !?!?!!' "Unable to find Void-Lotus's dots version. Exiting."
  exit 1
fi

# GitHub URL - VoidLotus's dots
branch="main"
github_url="https://github.com/Void-Lotus/Hyprland-Dots/tree/$branch/config/hypr/"
...
# Fetch the version from GitHub URL - VoidLotus's dots
github_version=$(curl -fsSL -A "Mozilla/5.0" "$github_url" | grep -o 'v[0-9]\+\.[0-9]\+\.[0-9]\+' | sort -V | tail -n 1 | sed 's/v//')

# Cant find  GitHub URL - VoidLotus's dots version
if [ -z "$github_version" ]; then
  notify-send -i "$iDIR/error.png" 'VoidLotus Hyprland:' "Unable to determine GitHub version."
  exit 1
fi

# Comparing local and github versions
if [ "$(echo -e "$github_version\n$local_version" | sort -V | head -n 1)" = "$github_version" ]; then
   notify-send -i "$iDIR/note.png" "VoidLotus Hyprland:" "No update available"
  exit 0
else
  # update available
  ...
  response=$($notify_cmd_shot "VoidLotus Hyprland:" "Update available! Update now?")

  case "$response" in
    "action1")  
      if [ -d "$VoidLotus_Dots_DIR" ]; then
      	if ! command -v kitty &> /dev/null; then
  			notify-send -i "$iDIR/error.png" "E-R-R-O-R" "Kitty terminal not found. Please install Kitty terminal."
  			exit 1
		fi
        kitty -e bash -c "
          cd \"$VoidLotus_Dots_DIR\" &&
          git stash &&
          git pull &&
          ./copy.sh &&
        ...
        kitty -e bash -c "
          git clone --depth=1 https://github.com/Void-Lotus/Hyprland-Dots.git $VoidLotus_Dots_DIR &&
          cd \"$VoidLotus_Dots_DIR\" &&
          chmod +x copy.sh &&
          ./copy.sh &&
		  notify-send -u critical -i "$iDIR/ja.png" 'Update Completed:' 'Kindly log out and relogin to take effect'
